...
After a browser-development callback, LivePulse redirects to `FRONTEND_BASE_URL` (local default `http://127.0.0.1:5173`) with a bounded result query. Packaged Tauri starts the same fixed backend OAuth route with the enumerated desktop completion mode bound to one-use OAuth state; a valid callback serves a static, secret-free local completion page and the desktop observes persisted connection state with bounded polling. Denial and other failures have safe completion/redirect outcomes. The OAuth callback URI remains the separately configured backend URL above. Provider-supplied error text is never reflected, and no arbitrary redirect target is accepted.
...
After environment setup, connect from the LivePulse UI and approve the requested playback scopes in the system browser. Browser development returns to the configured frontend; packaged desktop displays a local completion page and observes the stored connection. Then inspect `GET /api/v1/providers/spotify/connection` and `GET /api/v1/providers/spotify/playback`, and exercise a player command while a Spotify Connect device is active. Provider tests use mocked HTTP and do not require an account.
