...
After callback, LivePulse redirects to `FRONTEND_BASE_URL` (local default `http://127.0.0.1:5173`) with a bounded result query. The OAuth callback URI remains the separately configured backend URL above. Denial, invalid state, incomplete authorization, and exchange failures return to the frontend with a safe reason code; provider-supplied error text is never reflected.
