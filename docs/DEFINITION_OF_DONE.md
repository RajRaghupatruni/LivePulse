...
| Tauri packaging readiness boundary | **Prepared; packaging deferred** — an injected platform adapter supports endpoint configuration, external opening, fullscreen, preference storage, and backend lifecycle reporting. Browser-only fallbacks to replace later are documented below; no native code is claimed |
...
The later Tauri packaging pass must replace or supply adapters for browser `window.open` external opening, OAuth/location return navigation, the Fullscreen API, API/WebSocket endpoint configuration, localStorage preferences, sessionStorage wake-once policy, and browser lifecycle assumptions. The shell will own sidecar start/readiness/shutdown, window bounds/fullscreen/borderless mode, preferred-monitor placement, persisted geometry, native external URL/app launches, and packaging. The React UI must continue to use one window for product navigation. A Tauri WebView origin must be integrated with the PERSONAL_LOCAL host/origin trust boundary deliberately; do not widen that boundary to work around transport or WebSocket failures.
