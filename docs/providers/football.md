...
Provider event minutes are combined with the fixture's actual kickoff time when available. Otherwise the scheduled kickoff plus elapsed minutes is used as the best available UTC occurrence-time estimate. Poll observation time is retained separately. Canonical payloads keep the event minute separate from the provider's current observed match minute so the projection can retain a fresh minute while timeline occurrence remains accurate. Late events can be appended with their earlier occurrence time; stale lower-minute snapshots cannot regress stored score or lifecycle. The event and matching outbox record are persisted through the M1 repository in the same transaction as checkpoint advancement. The provider never writes to `match_state` or `pulse_timeline`; the existing projector remains authoritative. Fixture discovery is exposed as the read-only `GET /api/v1/football/fixtures` response with `today`, `upcoming`, and `live` lists and normalized `state` / `phase` fields. The provider's display label is not used to infer lifecycle state.

When no demo match is explicitly active, `/api/v1/live-state` recovers the newest durable live/halftime match projection. This lets browser refresh and WebSocket resynchronization reconstruct real provider state rather than returning an idle demo-only snapshot. Upcoming fixtures remain discovery snapshots; the canonical projection is preferred when its lifecycle is ahead of a stale fixture snapshot.

The first successful fixture-ingestion transaction stores all fixture checkpoints and `fixture-baseline:v1`. A successful empty fixture sync stores a zero-fixture marker as well, so a later arrival is recognized as new. Fixtures already present in a non-empty initial baseline do not produce scheduled timeline events merely because the provider returned them. The fixture query surface is still populated, and meaningful live facts (kickoff, goals, cards, phases, and corrections) continue to emit normally. Once the marker exists, a genuinely unseen fixture emits its scheduled event. Existing per-fixture checkpoints on an installation upgraded before the marker existed count as proof that baseline synchronization already happened. Baseline metadata and fixture checkpoints commit atomically, so a failed transaction retries without partially advancing the baseline.
