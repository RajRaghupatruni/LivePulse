...
The React UI depends on the shell-agnostic `web/src/lib/platform.ts` service for external launching, authorization entry, fullscreen requests, preference access, local API/WebSocket URL construction, backend lifecycle observations, and the in-app route seam. Browser behavior is implemented only in that service; feature components do not call native APIs or hard-code API hosts. Product navigation uses one in-window React surface; browser tabs are reserved for intentional external destinations. Browser development uses same-origin Vite proxies by default and may set `VITE_API_BASE_URL` / `VITE_WEBSOCKET_BASE_URL`; a desktop adapter must supply the bundled sidecar's API and WebSocket endpoints (or deliberate build-time endpoints), and transport fails closed if those endpoints are missing instead of targeting the WebView origin. No Tauri/Rust, packaging, monitor placement, or sidecar lifecycle is implemented in this milestone.

The UI treats backend readiness as runtime state rather than assuming a process was manually started. Browser checks show `starting` during a bounded initial grace, then `unavailable` with bounded retries; successful REST health, shell-reported readiness, and realtime connection remain separate observations. A desktop shell can synchronously expose a lifecycle snapshot before React mounts and subscribe to startup/shutdown changes; its `ready` observation should follow its readiness check. If the shell reports ready but the health request fails, the UI reports the backend unavailable. Preference access is synchronous so a shell can hydrate an in-memory store before mounting React, while browser development persists preferences through localStorage and the wake-once marker through sessionStorage. The desktop milestone must replace browser-only URL opening, OAuth return navigation, fullscreen, preference/wake storage, and endpoint configuration, and own sidecar start/readiness/shutdown, monitor placement, remembered window geometry, and packaging. Preferred-monitor behavior belongs to that shell. Internal product navigation stays inside the single LivePulse window.
