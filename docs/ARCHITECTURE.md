...
The Tauri v2 Rust host is deliberately small. It registers the URL opener and window-state plugins, sets a sensible resizable window size, and restores remembered geometry only when the previous monitor placement still intersects an available monitor. Its capability grants only the window fullscreen/maximize commands and HTTP(S) external URL opening; it does not grant arbitrary shell execution, filesystem access, or provider credentials. Standard Windows close behavior does not stop Compose, and backend/outbox/provider data remains durable. Packaged Spotify/Gmail OAuth opens a fixed loopback start URL in the system browser; an enumerated `desktop` completion mode is bound to each provider's one-use state and returns a static no-store page from FastAPI. The desktop shell polls safe provider connection status with a deadline, while browser OAuth retains its frontend redirect. Callback URIs, provider scopes, encrypted token storage, and the PERSONAL_LOCAL Host/Origin boundary are unchanged; no custom scheme or renderer-selected redirect is accepted.
