...
| Spotify current playback and controls | **Implemented** with OAuth, encrypted credentials, adaptive playback polling, playback/device endpoints, typed commands, and packaged desktop browser handoff; requires local credentials and an authorized Premium account |
...
| Adaptive command-center UI | **Implemented in final UI milestone**; provider-backed, reference-led landscape/portrait compositions and state-aware surfaces run in browser and Tauri |
...
| Failure tests | **Implemented selectively across M1–v1**; transaction rollback, transient publish retry, duplicate/concurrent delivery, stale versions, WebSocket recovery, consumer batch commit ordering, health normalization, provider checkpoint recovery, and focus expiry are tested; process-kill and prolonged broker restart injection remain future work |
...
The final React UI runs locally for `PERSONAL_LOCAL` in both the browser development/fallback host and the Windows-first Tauri v2 desktop shell. The UI targets fullscreen landscape at 1440×900, 1600×900 and 1920×1080, plus the locked 1080×1920 portrait composition. The operating environment integrates System Pulse, clock/weather, football Match Mode, the universal Pulse Timeline, read-only Gmail, Spotify, Focus Timer, Quick Launch and the CSS/SVG Pulse Field. Provider surfaces remain truthful and useful when optional credentials are absent; startup, backend readiness, reconnecting, and recovery are visible.

`web/src/lib/platform.ts` centralizes external launch, fullscreen/window controls, authorization entry, preferences, backend lifecycle observations, and local transport behind an injected platform interface. Browser development supplies browser fallbacks; the Tauri adapter provides system-default HTTP(S) URL opening, fullscreen/window controls, and explicit loopback API/WebSocket endpoints. Tauri checks `/health/ready` before showing the dashboard and does not own service process lifecycle. Docker Compose remains required for PostgreSQL, Redpanda, and FastAPI. The shell grants no arbitrary shell or filesystem access. Packaged Spotify and Gmail OAuth use fixed backend start URLs, system-browser consent, one-use desktop completion state, a static secret-free backend completion page, and bounded desktop connection polling; browser OAuth retains its frontend redirect behavior. Self-contained services, auto-update, and installer signing remain future work.
...
The desktop UI includes a centralized semantic visual-event layer. Canonical timeline and state transitions can drive restrained motion for football, Gmail, GitHub, Spotify, provider recovery, Focus, Match Mode, and selected weather. Historical events do not replay as new arrivals; repeated equivalent snapshots are deduplicated. A single lazy-loaded React Three Fiber atmosphere provides environmental depth while readable UI remains DOM. Adaptive quality, hidden-window suspension, and reduced-motion behavior are implemented. No formal long-duration GPU/FPS benchmark is claimed.
...
The Windows-first Tauri v2 desktop-shell milestone bundles the existing React/Vite production UI, adds a readiness/retry gate, the exact packaged Windows-origin allowance, scoped external HTTP(S) opening, fullscreen/window controls, and safe remembered geometry. Docker Compose remains the local service substrate; the desktop window detects readiness but does not start or stop containers. Browser development remains supported. This does not add providers, AI, self-contained services, auto-update, or code signing.
